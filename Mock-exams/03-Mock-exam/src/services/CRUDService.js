...
async function getLikes(bookId) {
    return await request.get(`${endpoints.likesUrl}?where=bookId%3D%22${bookId}%22&distinct=_ownerId&count`);
}

async function getFilteredLikes(bookId) {
    return await request.get(`${endpoints.likesUrl}?where=bookId%3D%22${bookId}%22%20and%20_ownerId%3D%22${authService.getUserData().userId}%22&count`);
}
